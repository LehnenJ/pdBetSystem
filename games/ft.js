...
(function() {
	...
	*/
		...
		*/
		if (command.equalsIgnoreCase('ft')) {
			...
			*/
				...
				$.say($.whisperPrefix(sender) + $.lang.get('pdbetting.notification.timer.set.' + (betTimerStatus ? 'on' : 'off'), betTimerSeconds));
				return;
	...
	function closeBet(sender) {
		...
		$.say($.whisperPrefix(sender) + $.lang.get('pdbetting.notification.bet.closed'));
	...
	function printInfo(sender, verbose) {
		...
		$.say($.whisperPrefix(sender) + $.lang.get('pdbetting.notification.bet.info.options', result.slice(0, -2)));
	};
